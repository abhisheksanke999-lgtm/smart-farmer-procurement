...
class ApiClient {
  ...
  async getDealerQueue() {
    return await this.request("/dealer/queue");
  }

  async startDealerQueueProcurement(token) {
    return await this.request("/dealer/queue/start", {
      method: "POST",
      body: JSON.stringify({ token_number: token })
    });
  }

  async completeDealerQueueProcurement(token) {
    return await this.request("/dealer/queue/complete", {
      method: "POST",
      body: JSON.stringify({ token_number: token })
    });
  }

  async nextDealerQueueFarmer() {
    return await this.request("/dealer/queue/next", {
      method: "POST"
    });
  }

  async resetDealerQueue() {
    return await this.request("/dealer/queue/reset", {
      method: "POST"
    });
  }
