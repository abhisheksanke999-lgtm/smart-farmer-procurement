...
class EmailDeliveryError(Exception):
    """Raised when email delivery fails via SMTP or Resend HTTP API."""
    pass
...
def send_via_gmail_smtp(to_email: str, recipient_name: str, otp_code: str, smtp_user: str, smtp_pass: str) -> Dict[str, Any]:
    """Sends OTP email directly using Gmail SMTP over TLS."""
    subject = "Your Smart Farmer Procurement Verification Code"
    html_body = generate_otp_html(recipient_name, otp_code)
    
    msg = MIMEMultipart("alternative")
    msg["Subject"] = subject
    msg["From"] = f"Smart Farmer Procurement <{smtp_user}>"
    msg["To"] = to_email
    ...
    try:
        ...
        logger.info(f"[GMAIL SMTP SUCCESS] OTP delivered to {to_email}")
        return {"status": "sent", "recipient": to_email, "provider": "gmail_smtp"}
    except Exception as e:
        logger.error(f"[GMAIL SMTP ERROR] Failed sending to {to_email}: {e}")
        raise EmailDeliveryError(f"Gmail SMTP Delivery Failed: {str(e)}")


def send_otp_email(to_email: str, recipient_name: str, otp_code: str) -> Dict[str, Any]:
    """
    Sends an OTP verification email using Gmail SMTP (if configured) or Resend HTTP REST API.
    Raises EmailDeliveryError if dispatch fails.
    """
    # Check if Gmail SMTP is configured
    smtp_user = os.environ.get("SMTP_USER", "").strip() or getattr(settings, "SMTP_USER", "").strip()
    smtp_pass = os.environ.get("SMTP_PASSWORD", "").strip() or getattr(settings, "SMTP_PASSWORD", "").strip() or os.environ.get("GMAIL_APP_PASSWORD", "").strip()

    if smtp_user and smtp_pass:
        return send_via_gmail_smtp(to_email, recipient_name, otp_code, smtp_user, smtp_pass)

    # Fallback to Resend HTTPS REST API
    resend_api_key = (
    ...
    if not resend_api_key:
        logger.error("[EMAIL CONFIG ERROR] Neither Gmail SMTP nor RESEND_API_KEY is configured.")
        raise EmailDeliveryError("Email service configuration error: Email credentials are not set.")
    ...
    try:
        ...
        if he.code == 401 or he.code == 403:
            ...
            raise EmailDeliveryError("Email authorization failed. Please check Resend API key.")
        elif he.code == 422:
